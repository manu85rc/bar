            ...
            @else
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ( as )
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ->numero }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ->capacidad }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ->ubicacion ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ ->disponible ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ ->disponible ? 'Sí' : 'No' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('mesas.edit', ) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Editar</a>
                                    <form action="{{ route('mesas.destroy', ) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('¿Estás seguro de eliminar esta mesa?');">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
