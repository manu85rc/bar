...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="border-4 border-dashed border-gray-200 rounded-lg h-full">
            <div class="mb-6">
                <h2 class="text-2xl font-semibold text-gray-900">Editar Categoría</h2>
            </div>
            ...
            <form action="{{ route('admin.categorias.update', $categoria->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('name', $categoria->name) }}" required>
                </div>
                ...
                <div>
                    ...
                    <select name="parent_id" id="parent_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        @foreach ($categorias as $cat)
                            <option value="{{ $cat->id }}" {{ old('parent_id', $categoria->parent_id) == $cat->id ? 'selected' : '' }} @if ($cat->id === $categoria->id) disabled @endif>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('description', $categoria->description) }}</textarea>
                </div>
